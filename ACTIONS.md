...
- [ ] **6.4** Team Profile / League Explorer pages — **not built**. Deferred: needs "list all teams in a league" and "upcoming fixtures" capabilities this system doesn't have yet (current fetchers are all built around "given one team name, get its recent matches," not league-wide browsing). Real scope, not a quick add — revisit as its own action once the core matchup analyzer is validated with real use.
